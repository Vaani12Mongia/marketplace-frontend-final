...
export default function Login() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // Store non-sensitive display info — includes role/actingTenant fields
      // so Sidebar.jsx can render the right view (master switcher vs normal
      // tenant nav) on first paint, before ProtectedLayout's own /me check lands
      sessionStorage.setItem('user_display', JSON.stringify({
        email:             session.email,
        companyName:       session.companyName,
        role:              session.role,
        actingTenantId:    session.actingTenantId,
        actingCompanyName: session.actingCompanyName,
      }))
