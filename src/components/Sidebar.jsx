...
//   const isActing = Boolean(isMaster && session?.actingTenantId)

//   // Master sees a tenant picker — load the list once we know who's logged in
//   useEffect(() => {
//     if (!isMaster) return
//     let mounted = true
//     api.listTenants()
//       .then(list => { if (mounted) setTenants(Array.isArray(list) ? list : []) })
//       .catch(() => { if (mounted) setTenants([]) })
//     return () => { mounted = false }
//   }, [isMaster])

//   // Agent dropdowns only make sense once there's an actual tenant in scope
//   // (a normal tenant session, or a master that has switched into one)
//   useEffect(() => {
//     if (isMaster && !isActing) { setLoading(false); return }
//     let mounted = true
//     ;(async () => {
//       try {
//         const agents = await api.listAgents()
...
//       window.location.href = '/'   // full reload clears any cached tenant-scoped state
//     } catch {}
//   }

//   const exitTenant = async () => {
//     try {
//       await api.exitTenant()
...
//             {isActing ? `Viewing: ${session.actingCompanyName}` : 'Master View'}
//           </div>
//           {isActing ? (
//             <button className="agent-select" onClick={exitTenant}>
//               ← Back to all tenants
//             </button>
//           ) : (
//             <select
...
export default function Sidebar() {
  ...
  const switchTenant = async (tenantId) => {
    ...
    try {
      await api.switchTenant(tenantId)
      window.location.href = '/app'   // full reload — re-runs ProtectedLayout's /me check, clears any cached tenant-scoped state
    } catch {}
  }
